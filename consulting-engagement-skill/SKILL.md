...
**Don't use for:** dbt model writing (use `dbt-skill`), DuckDB analytical queries (use `duckdb-local-skill`), DLT pipeline building (use `dlt-extraction-skill`), pipeline scheduling or orchestration (use `data-orchestration-skill`).

## Scope Constraints

- Consulting engagement lifecycle only: discovery, profiling, development, validation, delivery.
- Delegates to `dbt-skill` for transformation code generation.
- Delegates to `duckdb-local-skill` for local analytical queries.
- Delegates to `dlt-extraction-skill` for pipeline building.
