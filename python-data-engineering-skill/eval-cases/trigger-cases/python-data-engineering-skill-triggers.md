# python-data-engineering-skill — Trigger Eval Cases
...
**Expected Skill:** python-data-engineering-skill
...
- **Fail:** Skill does not activate, or routes to dbt-skill because "transformations" are mentioned
...
- **Fail:** Skill does not activate because no library name was mentioned, or routes to integration-patterns-skill because the data comes from a vendor API
...
**Expected Skill:** python-data-engineering-skill or dbt-skill (depends on context)
...
- The SKILL.md states "Do NOT use for SQL-based dbt models (use dbt-skill)", so the skill must clarify intent before proceeding
- If the data lives in a warehouse and the user works with dbt, this belongs to dbt-skill; if the user is working with files or DataFrames in Python, this skill applies
...
- **Partial:** Skill activates and assumes Python without asking, providing a correct DataFrame solution but missing the dbt-skill alternative
...
**Expected Activation:** Yes (with caveats)
**Expected Skill:** python-data-engineering-skill (primary), integration-patterns-skill (secondary)

**Observation Points:**
- Pydantic validation and API extraction scripts are explicitly listed in python-data-engineering-skill's activation criteria
- However, "building a data pipeline that pulls from the Stripe API" also sounds like integration architecture, which is integration-patterns-skill territory (Stripe is listed as a supported SaaS platform there)
...
- The SKILL.md scope constraint says "DLT pipeline config: hand off to integration-patterns-skill" but this is a custom Python extraction, not a dlt pipeline

**Grading:**
- **Pass:** Skill activates and provides Pydantic validation patterns for API response objects, referencing the extraction patterns and data validation references, while noting that the overall Stripe integration architecture could benefit from integration-patterns-skill
- **Partial:** Skill activates and covers Pydantic validation well but does not acknowledge the integration-patterns-skill overlap for the broader pipeline design
- **Fail:** Skill does not activate and defers entirely to integration-patterns-skill, or activates but provides only generic Pydantic examples unrelated to API extraction
...
- The SKILL.md states "Do NOT use for SQL-based dbt models (use dbt-skill)" and the scope constraints reinforce: "SQL transforms in dbt: hand off to dbt-skill"
...
**Grading:**
- **Pass:** Skill does not activate; dbt-skill handles the incremental model configuration and merge strategy
- **Partial:** Skill activates briefly but immediately redirects to dbt-skill without providing Python code
...
**Expected Skill:** python-data-engineering-skill (PySpark DataFrame transforms), streaming-data-skill (streaming architecture + Kafka)
...
- However, Structured Streaming, Kafka, and micro-batch processing are streaming-data-skill territory per the scope constraint "Kafka/Flink streaming: hand off to streaming-data-skill"
...
- The skill should handle the DataFrame transform patterns (deserialization, schema validation, Delta write) while acknowledging the streaming architecture belongs to streaming-data-skill

**Grading:**
- **Pass:** Skill activates for the PySpark DataFrame transformation components (Avro deserialization, schema validation logic, Delta Lake write patterns) while noting that the overall Structured Streaming architecture, Kafka configuration, and micro-batch tuning should be handled by streaming-data-skill
- **Partial:** Skill activates and provides PySpark code but does not distinguish which parts belong to streaming-data-skill
- **Fail:** Skill does not activate because it detects "Kafka" and "Structured Streaming" and defers entirely to streaming-data-skill, or fully activates and provides streaming configuration advice outside its scope
...
- The SKILL.md scope constraint explicitly says "Dagster/Airflow orchestration: hand off to data-orchestration-skill"
...
**Grading:**
- **Pass:** Skill does not activate for the orchestration request and redirects to data-orchestration-skill, optionally noting that python-data-engineering-skill can help write the individual Polars transform functions that would be called within the Dagster assets
- **Partial:** Skill activates and provides the Polars transform code while redirecting the Dagster orchestration portions to data-orchestration-skill
