# data-pipelines — Trigger Eval Cases
...
**Expected Skill:** data-pipelines (primary), dbt-transforms (possible secondary for dbt Cloud native scheduling)
...
- dbt-transforms covers dbt Cloud job configuration; data-pipelines covers external orchestrators running dbt
- The ambiguity hinges on whether the user has an external orchestrator or wants dbt Cloud's native scheduling

**Grading:**
- **Pass:** Skill activates and addresses scheduling dbt runs via an external orchestrator (dagster-dbt or cosmos), while noting that dbt Cloud native scheduling is an alternative handled by dbt-transforms's CI/CD reference
- **Partial:** Skill activates but only covers external orchestration without acknowledging the dbt Cloud alternative
- **Fail:** Skill does not activate (deferring entirely to dbt-transforms), or activates and tries to write dbt model SQL
...
**Observation Points:**
- DLT source configuration, pagination, and rate limiting are data-integration concerns
- SKILL.md explicitly states: "Don't use for: DLT source/destination config (use data-integration)"
...
**Grading:**
- **Pass:** Skill does not activate; defers to data-integration for DLT source configuration, pagination, and rate limiting
- **Partial:** Skill partially activates and offers to help with the orchestration layer (scheduling the DLT pipeline) while deferring source config to data-integration
...
- SKILL.md Scope Constraints: "Hand off transformation logic to dbt-transforms"
...
**Grading:**
- **Pass:** Skill does not activate; dbt-transforms handles the transformation request
- **Partial:** Skill does not activate but incorrectly suggests data-pipelines might be needed later without being asked
...
**Expected Skill:** data-pipelines (primary), data-integration (for DLT config), dbt-transforms (for dbt model logic), ai-data-integration (not involved)
...
- data-pipelines owns the Dagster pipeline structure, scheduling, dagster-dbt integration, dagster-dlt integration, and Slack alerting
- The DLT ingestion configuration and dbt model logic are handoffs to sibling skills
- Elementary data quality checks could be orchestrated by this skill but the check definitions belong to dbt-transforms's data quality reference
...
- SKILL.md Don't use for: "Kafka/Flink streaming (use event-streaming)"
- Kafka consumer groups, exactly-once semantics, and schema evolution are core event-streaming topics
...
**Grading:**
- **Pass:** Skill does not activate despite being explicitly named; redirects to event-streaming for Kafka consumer configuration, exactly-once semantics, and schema evolution
