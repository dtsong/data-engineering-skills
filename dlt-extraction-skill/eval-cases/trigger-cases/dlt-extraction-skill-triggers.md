# dlt-extraction-skill -- Trigger Eval Cases
...
**Expected Skill:** dlt-extraction-skill
...
- Skill does not defer to integration-patterns-skill since the request is file-based, not API or database
...
- **Fail:** Skill does not activate, or integration-patterns-skill claims the prompt because it also covers DLT
...
- **Fail:** Skill does not activate because no DLT-specific language is present, or python-data-engineering-skill activates for generic Python file processing
...
**Expected Activation:** Yes (if file-based context); integration-patterns-skill (if REST API/database context)
**Expected Skill:** dlt-extraction-skill or integration-patterns-skill depending on context

**Observation Points:**
- "DLT pipeline for data extraction" is ambiguous -- could be file-based (this skill) or API/database (integration-patterns-skill)
...
- If prior conversation mentions REST APIs, databases, or SaaS platforms, integration-patterns-skill should claim it
...
- The SKILL.md explicitly states "Do NOT use for core DLT concepts like REST API or SQL database sources (use integration-patterns-skill)"
- The presence of "DLT pipeline" should not cause this skill to claim the prompt when the source is a REST API
- integration-patterns-skill covers REST API DLT sources in its dlt-pipelines reference

**Grading:**
- **Pass:** Skill does not activate; integration-patterns-skill handles the REST API DLT pipeline request using its dlt-pipelines reference and REST API source patterns
- **Partial:** Skill activates briefly but immediately redirects to integration-patterns-skill for REST API source guidance
...
- No other skill covers this combination -- integration-patterns-skill handles REST APIs, not file-based SharePoint extraction
...
- **Fail:** Skill does not activate, or integration-patterns-skill claims the prompt because SharePoint is a SaaS platform
...
- Writing dbt models is entirely within dbt-skill's domain, not dlt-extraction-skill's
...
**Grading:**
- **Pass:** Skill does not activate for dbt model writing and redirects to dbt-skill, optionally noting that dlt-extraction-skill handles the extraction layer that feeds into dbt transformations
- **Partial:** Skill activates and provides some guidance on how DLT pipelines connect to dbt models downstream, while redirecting the core dbt model writing to dbt-skill
