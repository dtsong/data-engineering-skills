# data-integration — Trigger Eval Cases
...
- Skill does not defer to data-pipelines despite the "every 15 minutes" scheduling language, since the core task is designing the integration itself
...
- **Fail:** Skill does not activate, or a different skill (e.g., data-pipelines) claims the prompt based on the scheduling language
...
- **Fail:** Skill does not activate because no exact trigger phrase is present, or routes to python-data-engineering because the user might write a custom script
...
**Expected Skill:** data-integration (primary), event-streaming (secondary)

**Observation Points:**
- "Capture every insert and update" maps directly to CDC, which is explicitly listed under data-integration activation criteria
- "Process them in near-real-time" introduces ambiguity — the processing step could involve stream processing frameworks (event-streaming territory)
- The skill should claim CDC capture as its domain but flag that the processing layer may require event-streaming handoff

**Grading:**
- **Pass:** Skill activates, proposes CDC with Debezium or similar as the capture mechanism, and acknowledges that the real-time processing step may require event-streaming if it involves complex transformations, windowing, or stateful processing
- **Partial:** Skill activates and covers CDC capture but does not surface the event-streaming boundary for the processing component
- **Fail:** Skill does not activate and defers entirely to event-streaming, or activates without any acknowledgment of the processing ambiguity
...
- **Fail:** Skill does not activate, or dbt-transforms activates because the data originates from a warehouse transformation
...
- The SKILL.md explicitly states "Do NOT use for stream processing frameworks (use event-streaming)"
- The presence of Kafka as an output sink should not cause data-integration to claim the prompt — the core task is stream processing, not system integration

**Grading:**
- **Pass:** Skill does not activate; event-streaming handles the request for Flink stateful processing and session windows
- **Partial:** Skill activates briefly but immediately redirects to event-streaming without attempting to provide Flink guidance
...
**Expected Skill:** data-integration (CDC + event architecture), event-streaming (Flink processing), python-data-engineering (Python loader)

**Observation Points:**
- This prompt spans at least three skills: CDC and event-driven architecture (data-integration), Flink stream processing (event-streaming), and Python data loading (python-data-engineering)
...
- The skill should acknowledge that Flink enrichment requires event-streaming and the Python loader may require python-data-engineering

**Grading:**
- **Pass:** Skill activates and covers the CDC capture pattern, Kafka event architecture, and overall integration design, while explicitly noting handoffs to event-streaming for Flink processing and python-data-engineering for the Python loader component
...
**Observation Points:**
- The user explicitly invokes "integration patterns skill" and mentions "dlt pipeline" (a legitimate data-integration topic)
- However, the actual request is entirely about orchestration: cron scheduling, dependency tracking, retry logic, and alerting
- The SKILL.md explicitly states "Do NOT use for pipeline scheduling (use data-pipelines)"
- The skill should not comply with the bypass attempt despite the explicit invocation and the dlt mention

**Grading:**
- **Pass:** Skill does not activate for the scheduling request and redirects to data-pipelines, optionally noting that data-integration can help with the dlt pipeline design itself (sources, schema contracts, incremental loading) but not with scheduling and orchestration
- **Partial:** Skill activates and provides some dlt pipeline design guidance while redirecting the scheduling portions to data-pipelines
