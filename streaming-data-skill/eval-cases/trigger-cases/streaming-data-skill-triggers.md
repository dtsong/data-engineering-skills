# streaming-data-skill — Trigger Eval Cases
...
- **Fail:** Skill does not activate, or integration-patterns-skill claims the prompt because Kafka is mentioned in its event-driven architecture scope
...
- **Fail:** Skill does not activate because no streaming technology is named, or routes to dbt-skill or data-orchestration-skill because "dashboard data" sounds like a transformation or scheduling concern
...
**Expected Skill:** streaming-data-skill (primary), integration-patterns-skill (secondary)

**Observation Points:**
- Kafka event consumption and routing is clearly streaming territory, matching "Building or troubleshooting Kafka pipelines"
- However, "route to different downstream services" also resembles event-driven integration architecture, which integration-patterns-skill covers under "Building event-driven architectures with Kafka, Pub/Sub, or EventBridge"
- The key differentiator: if the routing involves stream processing logic (filtering, enrichment, stateful decisions), it belongs to streaming-data-skill; if it is pure topic-based fan-out or integration orchestration, integration-patterns-skill may be more appropriate
- The mention of fraud detection hints at real-time processing needs, tipping toward streaming-data-skill

**Grading:**
- **Pass:** Skill activates and addresses the Kafka event routing pattern (topic partitioning, consumer groups per service, or stream processing fan-out), while noting that the overall event-driven architecture and service integration patterns could involve integration-patterns-skill
- **Partial:** Skill activates and provides a correct streaming solution but does not acknowledge the integration-patterns-skill overlap for the service routing architecture
- **Fail:** Skill does not activate and defers entirely to integration-patterns-skill, or activates without any consideration of which routing pattern best fits the three different consumer requirements
...
**Expected Activation:** Yes (with caveats)
**Expected Skill:** streaming-data-skill (primary), dbt-skill (secondary)

**Observation Points:**
- Materialized views are explicitly listed in the streaming-data-skill description, and Snowflake Dynamic Tables are covered in the warehouse-streaming-ingestion reference
...
- The phrase "automatically refresh as new data arrives" signals continuous/streaming semantics rather than scheduled dbt runs, which tips toward streaming-data-skill
- If the user is working within a dbt project, the dbt-skill may need to handle the model definition while streaming-data-skill handles the refresh architecture

**Grading:**
- **Pass:** Skill activates and addresses Snowflake Dynamic Tables or materialized views with continuous refresh patterns from the warehouse-streaming-ingestion reference, while noting that if the user manages these within a dbt project, the dbt-skill handles the model definition layer
- **Partial:** Skill activates and covers the streaming refresh mechanics but does not mention the dbt-skill overlap for users working in dbt projects
- **Fail:** Skill does not activate and defers to dbt-skill because "materialized views" is a dbt materialization strategy, or activates but provides only generic Kafka streaming advice instead of warehouse-native streaming guidance
...
**Expected Skill:** integration-patterns-skill (file-based integration) or data-orchestration-skill (scheduling)
...
- The SKILL.md explicitly states "Do NOT use for batch integration patterns (use integration-patterns-skill)"
...
**Grading:**
- **Pass:** Skill does not activate; integration-patterns-skill handles the file-based integration pattern and data-orchestration-skill handles the nightly scheduling
...
**Expected Skill:** streaming-data-skill (Flink streaming + architecture), dbt-skill or python-data-engineering-skill (batch Spark job), data-orchestration-skill (nightly scheduling)
...
- Flink for real-time clickstream processing is a direct match for the streaming-data-skill activation criteria
- However, the nightly batch Spark job and the merge logic span other skills: batch processing (dbt-skill or python-data-engineering-skill) and scheduling (data-orchestration-skill)
...
**Grading:**
- **Pass:** Skill activates and designs the streaming layer (Flink clickstream processing, live dashboard feed) and the overall Lambda architecture pattern, while explicitly noting that the batch recomputation job belongs to dbt-skill or python-data-engineering-skill and the nightly scheduling belongs to data-orchestration-skill
...
- The SKILL.md explicitly states "Do NOT use for pipeline orchestration (use data-orchestration-skill)"
...
**Grading:**
- **Pass:** Skill does not activate and redirects to data-orchestration-skill, explaining that Airflow DAG scheduling and dbt model orchestration are outside streaming-data-skill scope regardless of the cadence
- **Partial:** Skill activates and suggests that hourly processing could be handled by a streaming approach instead, but ultimately redirects to data-orchestration-skill for the actual request
