# ai-data-integration-skill — Trigger Eval Cases
...
**Expected Skill:** ai-data-integration-skill
...
- **Fail:** Skill does not activate, or integration-patterns-skill activates instead
...
- **Fail:** Skill does not activate because no explicit AI/LLM keywords appear, or integration-patterns-skill activates because it sees "data integration"
...
**Expected Skill:** ai-data-integration-skill (primary), python-data-engineering-skill (possible secondary)
...
- python-data-engineering-skill could handle this if the user wants a traditional NLP or regex approach

**Grading:**
- **Pass:** Skill activates and surfaces the LLM vs traditional trade-off, noting that messy free-text classification favors LLMs but 500K rows requires batching and cost estimation. Acknowledges python-data-engineering-skill as an alternative if the user prefers a non-LLM approach
...
**Expected Activation:** Yes (with caveats)
**Expected Skill:** ai-data-integration-skill (primary for NL-to-SQL layer), integration-patterns-skill (possible secondary for API architecture)

**Observation Points:**
- "Ask questions about our BigQuery data" implies NL-to-SQL, a core ai-data-integration-skill use case
...
- integration-patterns-skill could claim the API/architecture layer; dbt-skill could claim the semantic layer component

**Grading:**
- **Pass:** Skill activates for the NL-to-SQL and data access components, noting that application architecture (charting, frontend) is out of scope, and that dbt-skill's semantic layer may complement the solution
- **Partial:** Skill activates but tries to cover the full application stack including charting and frontend architecture
- **Fail:** Skill does not activate, or dbt-skill activates because it sees "business terms" without recognizing the NL-to-SQL intent
...
- **Partial:** Skill does not activate but fails to note that once the model is fine-tuned, ai-data-integration-skill could help with the data access layer
...
**Expected Skill:** ai-data-integration-skill (for LLM enrichment), integration-patterns-skill (for DLT ingestion), dbt-skill (for staging models), data-orchestration-skill (for Dagster scheduling)

**Observation Points:**
- This spans four skills; ai-data-integration-skill should activate for the LLM enrichment step only
...
**Grading:**
- **Pass:** Skill activates for the LLM enrichment component, provides classification patterns and cost guidance, and explicitly hands off DLT ingestion to integration-patterns-skill, dbt staging to dbt-skill, and Dagster scheduling to data-orchestration-skill
